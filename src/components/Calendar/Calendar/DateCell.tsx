...
export function DateCell({day, price, isSelected = false}: DateCellProps) {
    ...
    const selectedClasses = isSelected ? "bg-emerald-50 border-2 border-emerald-600" : "border border-gray-300";

    const dayClasses = `h-6 text-base text-center text-black ${isSelected ? "font-bold" : ""}`;
    const priceClasses = `h-6 text-base font-bold text-center ${isSelected ? "text-emerald-600" : "text-black"}`;
